...
# Define TF model graph for the detector
# This starts from the residual
detector = detector_model(img_rows=img_rows, img_cols=img_cols,
                          channels=nchannels,
                          nb_classes=2)
print('Defined TensorFlow detector graph.')
# Load weights
detector.load_weights(detector_path)
detector_wrap = KerasModelWrapper(detector)
...
# Input tensors for the image and wavelet-filtered outputs
x  = tf.placeholder(tf.float32, (batch_size, input_w, input_h, input_c))
xw = tf.placeholder(tf.float32, (batch_size, input_w, input_h, input_c))
# Construct residual as their difference
r = x - xw
...
# Residual for the modified image
r_mod = new_image - xw
...
# Detection for the modified image
det_logits_mod = detector_wrap.get_logits(r_mod)
det_probs_mod  = detector_wrap.get_probs(r_mod)
det_preds_mod  = tf.argmax(det_logits_mod, axis=-1)

# Reference tensor for the joint classifier-detector labels
y_joint = tf.placeholder(tf.float32, (batch_size, num_classes+1))
...
# Joint classifier-detector logit loss
# Construct the (C+1)-th logit by subtracting the two logits of the one-hot encoded detector network
extra_logit_mod = tf.expand_dims(tf.reduce_max(logits_mod, axis=-1) *
                                 (1 + det_logits_mod[:,1]-det_logits_mod[:,0]), axis=-1)
# Merge with classification logits
joint_logits = tf.concat([logits_mod, extra_logit_mod], axis=-1)

# Construct targeted joint logit loss
joint_logit_true  = tf.reduce_sum((y_joint * joint_logits), axis=-1)
joint_logit_other = tf.reduce_max((1 - y_joint) * joint_logits - (y_joint * 10000.), axis=-1)
max_loss_joint    = tf.maximum(0., -joint_logit_true + joint_logit_other + confidence)
# Lambda tensor - one for each element in batch
lam = tf.placeholder(tf.float32, (batch_size,))

# Loss function for joint-modifier approach
loss_joint = lam * max_loss_joint +  alpha * modifier_loss
grad_joint, = tf.gradients(loss_joint, modifier)

# Adam optimizer
start_vars = set(x.name for x in tf.global_variables())
optimizer = tf.train.AdamOptimizer(learning_rate=0.01)
trainer   = optimizer.minimize(loss_joint, var_list=[modifier])
...
y_clean_joint = np.append(y_clean, np.zeros((num_images, 1)), axis=-1)

# Roll each image to another random class
y_target_joint = np.copy(y_clean_joint)
for image_idx in range(num_images):
    y_target_joint[image_idx, :num_classes] = np.roll(y_clean[image_idx],
                  np.random.randint(low=1, high=10), axis=-1)
# Get natural target class
target_class_idx = np.argmax(y_target_joint, axis=-1)
...
for patch_idx in range(num_patches):
    ...
    # Start from upper bound, close to the input, and slowly move away
    batch_lambda = np.copy(lower_bounds)
    
    # For each lambda value
    for step_idx in range(lambda_steps):     
        # Initialize - restart from tclean images
        np_newimg = np.copy(x_clean)
        # Reset Adam's state
        sess.run(mod_init)
        # Create a filename
        filename = 'results/cifarVgg19_CWwhitebox_%s_patch%d_wavelets%d_iterations%d_confidence%.1f_step%d.mat' % (
                restricted_name, patch_idx, wavelet_interval, num_iterations, confidence, step_idx)
        
        # Local best distances
        past_x_dist = np.copy(local_x_dist)
                
        for i in range(num_iterations):
            # Wavelet-filtered image batch
            adv_mod_img = np_newimg
            # Compute wavelet transform periodically
            if np.mod(i, wavelet_interval) == 0:
                _, adv_mod_img_def = get_residual(adv_mod_img, train_mean, train_std,
                                                  sigma=test_sigma)
                    
            # Dictionary for modifier attack
            feed_dict_mod = {xw: adv_mod_img_def,
                         x: x_clean_tanh, # We always feed the clean image in this, since modifier is separate
                         l2_orig: x_clean_tanh,
                         y_joint: y_target_joint,
                         lam: batch_lambda,
                         cornerx: cornerx_np,
                         cornery: cornery_np}
        
            # Run attack for modifier
            _, g_mod, mod_loss, p_mod, det_p_mod, np_logits_mod, np_det_logits, \
            np_loss_joint, np_max_loss_joint, np_joint_logits, np_newimg, \
            np_joint_logit_true, np_joint_logit_other = sess.run([trainer, grad_joint, modifier_loss,
                                      preds_mod, det_preds_mod,
                                      logits_mod, det_logits_mod, loss_joint, max_loss_joint, joint_logits, new_image,
                                      joint_logit_true, joint_logit_other],
                                                                                feed_dict_mod)
           
            # Record best found images
            # These points will have exactly zero loss
            local_success_idx = np.where(np_max_loss_joint == 0)[0]
